...
class nrpe (
  ...
  if $individualpluginspackages != '' {
    package { $individualpluginspackages : ensure => present }
  } elsif ! defined(Package[$nrpe::params::pluginspackage]) {
    package { $nrpe::params::pluginspackage : ensure => present }
  }

  if $ownplugins != '' {
    nrpe::ownplugins{$ownplugins:}
  }
